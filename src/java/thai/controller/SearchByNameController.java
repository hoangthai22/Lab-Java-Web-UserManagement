...
public class SearchByNameController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = ERROR;
        try {
            String key = request.getParameter("txtSearchByName");
            if (!key.equals("")) {
                UserDAO dao = new UserDAO();
                List<UserObj> listUser = dao.findUserByPrimaryKey(key, "");
                if (listUser.size() > 0) {
                    request.setAttribute("listSearchAll", listUser);
                }
            } else {
                request.setAttribute("error", "Please enter the keyword you want to search!");
            }
            url = GETALL;
        } catch (Exception e) {
            log("ERROR at SearchByNameController: " + e.getMessage());
            LOGGER.info("ERROR at SearchByNameController:" + e.getMessage());
        } finally {
